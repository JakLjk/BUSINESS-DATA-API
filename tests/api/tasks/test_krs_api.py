...
def test_odpis_aktualny_invalid_krs_characters():
    ...
    with pytest.raises(InvalidParameterException):
        krs_api.get_odpis_aktualny(krs=krs, rejestr="P")
...
def test_odpis_aktualny_invalid_rejestr_characters():
    ...
    rejestr = "X"
    with pytest.raises(InvalidParameterException):
        krs_api.get_odpis_aktualny(krs=VALID_KRS, rejestr=rejestr)
...
def test_odpis_aktualny_valid():
    krs_api = KRSApi()
    response = krs_api.get_odpis_aktualny(krs=VALID_KRS, rejestr="P")
...
def test_odpis_pelny_invalid_krs_characters():
    ...
    krs = "abc1234567"
    with pytest.raises(InvalidParameterException):
        krs_api.get_odpis_pelny(krs=krs, rejestr="P")
...
def test_odpis_pelny_invalid_rejestr_characters():
    ...
    rejestr = "X"
    with pytest.raises(InvalidParameterException):
        krs_api.get_odpis_pelny(krs=VALID_KRS, rejestr=rejestr)
...
def test_odpis_pelny_valid():
    krs_api = KRSApi()
    response = krs_api.get_odpis_pelny(krs=VALID_KRS, rejestr="P")
